(function ( $, window, document, undefined ) {
  ...
  ComboTree.prototype.init = function () {
    ...
    // 29/09/2021: nd-thinh custom to set selected when single select
    if (this.options.isMultiple) {
        this._elemInput.wrap('<div id="'+ this.comboTreeId + 'Wrapper" class="comboTreeWrapper"></div>');
    } else {
        this._elemInput.wrap('<div id="'+ this.comboTreeId + 'Wrapper" class="comboTreeWrapper comboTreeSingleWrapper"></div>');
    }
  ...
  ComboTree.prototype.createFilterHTMLForMultiSelect = function (){
    return '<input id="' + this.comboTreeId + 'MultiFilter" type="text" class="multiples-filter" placeholder="Nhập từ khóa để tìm kiếm"/>';
  }
  ...
  ComboTree.prototype.processSelected = function () {
    ...
    this.options.selected.forEach(function(element) {
      ...
      // 24/9/2021 nd-thinh: custom to set color for selected item
      selected.addClass("comboTreeItemHover");
    ...
    // Disable category id and all child
    var categoryId = this.options.categoryId;
    if (categoryId > 0) {
      var $category = $("span.comboTreeItemTitle[data-id='" + categoryId +"']");
      $category.addClass('disabled');
      var $childs = $category.parent('li').children('ul');
      $childs.each(function() {
        var $child = $(this);
        $child.find('span.comboTreeItemTitle').addClass('disabled');
      });
    }
